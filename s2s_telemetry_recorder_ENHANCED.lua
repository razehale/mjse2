...
local function s2s_stop_recording()
    ...
    s2s_log("INFO", "Recording STOPPED  (" .. tostring(s2s_sample_count) ..
           " samples) → " .. s2s_filepath)
end

local function s2s_start_recording()
    ...
    s2s_filepath = dir .. "s2s_telemetry_" .. ts .. ".csv"
    ...
    s2s_log("INFO", string.format("Config: %dHz, %d columns, REP=%s, v%s",
            SAMPLE_HZ, NUM_COLS, tostring(S2S_REP_DETECTED), VERSION))
...
function s2s_frame_callback()
    ...
    local ok, errmsg = pcall(function()
        ...
        local row = string.format(
            ...
            S2S_on_ground or 0,            S2S_park_brake or 0,
            S2S_fuel_kg or 0,
...
function s2s_draw_overlay()
    ...
    -- START ST-960: Added version and Hz to overlay
    local status   = string.format("● REC v%s  %02d:%02d  [%d @ %dHz]",
                                   VERSION, mins, secs, s2s_sample_count, SAMPLE_HZ)
    -- END ST-960
...
s2s_log("INFO", "s2s_telemetry_recorder.lua loaded  (v" .. VERSION ..
        " – ST-960 ENHANCED: mag heading + unit fixes + REP L1 + stall monitor)")
